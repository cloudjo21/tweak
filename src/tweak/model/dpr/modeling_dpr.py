...
@dataclass
class DPRForPreTrainingOutput(ModelOutput):
...
class DPRForPreTraining(DPRPreTrainedModel):
    def __init__(self, ctx_config, question_config):
        super().__init__(ctx_config, question_config)

        self.c_encoder = DPRContextEncoder(ctx_config)
        self.q_encoder = DPRQuestionEncoder(question_config)
    ...
    def forward(
        ...
        context_attention_mask: Optional[Tensor] = None,
        context_token_type_ids: Optional[Tensor] = None,
        context_inputs_embeds: Optional[Tensor] = None,
        positive_labels: Optional[Tensor] = None,
        negative_labels: Optional[Tensor] = None,
        output_attentions: bool = False,
        output_hidden_states: bool = False,
        return_dict: bool = False
    ) -> Union[Tuple[torch.Tensor], DPRForPreTrainingOutput]:
        q_output = self.q_encoder(
            input_ids,
            attention_mask,
            token_type_ids,
            inputs_embeds,
        ...
        if not return_dict:
            ...
            q_vector = q_output.pooler_output
            c_vector = c_output.pooler_output

        product_scores = torch.matmul(q_vector, c_vector.transpose(0, 1))
        # product_scores = product_scores.view(product_scores.size(0), -1)

        logits = torch.nn.functional.log_softmax(product_scores, dim=1)
        print(logits.shape)
        print(positive_labels.shape)
        print(positive_labels)
        # positive_labels = positive_labels.to(logits.device)
        total_loss = None
        if positive_labels is not None:
            total_loss = torch.nn.functional.nll_loss(
                logits,
                positive_labels,
                reduction='mean'
            )
        ...
        if not return_dict:
            return (total_loss, logits) + q_output[2:] + c_output[2:]

        return DPRForPreTrainingOutput(
            loss=total_loss,
            prediction_logits=logits,
            hidden_states=q_output.hidden_states,
            attentions=q_output.attentions,
            context_hidden_states=c_output.hidden_states,
            context_attentions=c_output.attentions
        )
...
class DPREncoder(DPRPreTrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            return (sequence_output, pooled_output) + outputs[0:]
