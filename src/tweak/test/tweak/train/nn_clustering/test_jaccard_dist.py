...
class JaccardDistTest(unittest.TestCase):
    ...
    def test_search_and_tokenize(self):
        ...
        es_results = self.es.search(query, index=self.index_name)
        ...
        dist_calc_req = JaccardDistanceCalcRequest(term_vector= term_vector, num_rows=self.max_row) # TODO set config from deploy
