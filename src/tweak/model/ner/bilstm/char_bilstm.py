...
class CharBiLSTM(nn.Module):
    ...
    def forward(
        ...
        char_seq_tensor = char_seq_tensor.reshape(batch_size * sent_len, -1)
        char_seq_len = char_seq_len.reshape(batch_size * sent_len)
        ...
        hidden = (
            char_hidden[0]
            .transpose(1, 0)
            .reshape(batch_size * sent_len, 1, -1)
        )  ### before view, the size is ( batch_size * sent_len, 2, lstm_dimension) 2 means 2 direciton..
        return hidden[recover_idx].reshape(batch_size, sent_len, -1)
